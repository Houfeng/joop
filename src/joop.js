...
(function(owner) {
    ...
    owner.$namespace = function(_namespace, _class) {
    ...
    owner.$using = function(_namespace, _shortName) {
        return $namespace(_shortName, $namespace(_namespace));
    }

    /**
     * 引入一个外部脚本 (require)
     * @param  {Array or String}   pathList    一个或多个外部脚本
     * @param  {Function}          callback   引入成功回调
     * @param  {String}            importType 引入的文件类型（script,style）
     * @return {void}              无返回值
     */

    owner.$import = function(pathList, callback, importType) {
        importType = importType || "script"
        if ($helper.isString(pathList)) pathList = [pathList];
        //完成计数器
        var readyCount = 0;
        //pathList
        $helper.each(pathList, function() {
            var _url = $import.handlePath(this);
            ...
            var _element = null;
            if (importType == "style" || importType == "css") {
                _element = $import.createStyle(_url);
            } else {
                _element = $import.createScript(_url);
            }
            _element.callbackList = [];
            if (callback) _element.callbackList.push(callback);
            _element.load = _element.onload || _element.onreadystatechange;
            _element.load = $event(_element, 'load');
            _element.load(function() {
                var _state = this.readyState || "loaded";
                if (_state == "loaded" || _state == "interactive" || _state == "complete") {
                    _element.ready = true;
                    readyCount++;
                    if (readyCount >= pathList.length) {
                        $helper.each(_element.callbackList, function(i) {
                            if (_element.callbackList[i]) _element.callbackList[i]();
                            _element.callbackList[i] = null;
                        });
                    }
                }
            });
            $import.cache[_url] = _element;
            $import.addElement(_element);
        });
    };
    /**
     * 引入缓存，用来防止重复引入
    ...
    /**
     * 将一个引入文件元素添加到页面
     * @param  {Element} _element 元素
     * @return {void}             无返回值
     */
    owner.$import.addElement = function(_element) {
     ...
     * @type {Object}
     */
    owner.$import.basePath = (function() {
        var _scripts = document.getElementsByTagName("script");
        var _basePath = '';
        $helper.each(_scripts, function() {
            _basePath = this.basePath || this.getAttribute('basePath') || _basePath;
        });
        return _basePath;
    })();
    /**
     * 处理引入的文件路径
     * @param  {String} _path 原始文件路径
     * @return {String}       处理后的文件路径
     */
    owner.$import.handlePath = function(_path) {
        if ($helper.contains(_path, "http://")) return _path;
        else return $import.basePath + _path;
    };
    /**
     * 创建一个Script元素
     * @param  {String} _url URL
     * @return {Element}     Script元素
     */
     ...
     */
    owner.$import.createStyle = function(_url) {
